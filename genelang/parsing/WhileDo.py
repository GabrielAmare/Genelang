...
class WhileDo(Process):
    ...
    def build(self, parser, tokens: TokenList, at_position: int) -> DoWhileResult:
        while_do_result = DoWhileResult(self, at_position)

        while True:
            do_result = self.left.build(parser, tokens, at_position)

            while_do_result.append(do_result)
            ...
            while_do_result.append(while_result)

        return while_do_result
